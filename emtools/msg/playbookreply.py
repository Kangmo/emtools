...
class PlaybookReply(JsonMsg):
    ...
    struct RecapInfo {
        1: required int     ok;
        2: required int     changed;
        3: required int     unreachable;
        4: required int     failed;
    }
    
    struct PlaybookReply {
        1: required string  cluster_name;             // cluster name
        2: required PlaybookInfo playbook_info;       // playbook info
        3: required int     rc;                       // command return code
        4: required string  stdout;                   // command stdout
        5: required string  stderr;                   // command stderr
        6: optional map<string,RecapInfo> recap_info  // recap summaries
    }
...
def PlaybookReply_from_dict( inv_dict ):
    return PlaybookReply( json.dumps( inv_dict ) )